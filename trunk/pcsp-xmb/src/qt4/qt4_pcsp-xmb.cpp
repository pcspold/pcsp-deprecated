	...
	d.setFilter(QDir::Files);
     ...
     for (int i = 0; i < list.size(); ++i) {
		 ...
		 if (((suffix != "CSO") && (suffix != "ISO")))//no iso found
			 continue;
		  QString fullpath = m_umdisospath + "/" + fileInfo.fileName();
...
void pcspfrontend::menu_changeUmdPath()
{
     ...
     QString directory = QFileDialog::getExistingDirectory(0,
                                 ...
                                 "umdimages/",
                                 options);
	 m_umdisospath=directory;//change the directory to the new one
	 QSettings settings("settings.ini", QSettings::IniFormat);//save the new path
	 settings.setValue("umdpath",m_umdisospath);
     clearFormComponents();
	 refreshBrowser();
}
